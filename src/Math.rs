use std::{mem::size_of, num::ParseIntError, fmt::{Display, Debug}};

use num::{ToPrimitive, traits::AsPrimitive, iter::RangeInclusive, PrimInt, integer::Roots};
use rand::Rng;
...
{
    let two = T::one() + T::one();
    num::range_inclusive(two, val.sqrt())
    .all(|i| val % i != T::zero())
}

pub fn prime3(val: usize) -> bool {
    num::range_inclusive(2, val.sqrt())
    .all(|i| val % i != 0)
}
